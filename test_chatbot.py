...
TEST_CASES: List[TestCase] = [
    ...
    # ============================================================
    #  MỞ RỘNG v2 (+35 kịch bản, nâng 15 -> 50) — bám sách/tác giả THẬT
    # ============================================================
    # ── FACTUAL (+12) ── keyword = tên sách + tác giả (kỳ vọng có trong câu trả lời)
    TestCase(case_id="TC_F05", query="Lão Hạc của Nam Cao kể về điều gì",
             case_type="FACTUAL", expect_keywords=["lão hạc", "nam cao"]),
    TestCase(case_id="TC_F06", query="Truyện Kiều của Nguyễn Du nội dung chính là gì",
             case_type="FACTUAL", expect_keywords=["truyện kiều", "nguyễn du"]),
    TestCase(case_id="TC_F07", query="Vợ nhặt của Kim Lân viết về nạn đói năm nào",
             case_type="FACTUAL", expect_keywords=["vợ nhặt", "kim lân"]),
    TestCase(case_id="TC_F08", query="Dế Mèn phiêu lưu ký của Tô Hoài dành cho ai",
             case_type="FACTUAL", expect_keywords=["dế mèn", "tô hoài"]),
    TestCase(case_id="TC_F09", query="Cánh đồng bất tận là tác phẩm của ai",
             case_type="FACTUAL", expect_keywords=["cánh đồng bất tận", "nguyễn ngọc tư"]),
    TestCase(case_id="TC_F10", query="Mắt biếc của Nguyễn Nhật Ánh nói về chuyện gì",
             case_type="FACTUAL", expect_keywords=["mắt biếc", "nguyễn nhật ánh"]),
    TestCase(case_id="TC_F11", query="Giông tố của Vũ Trọng Phụng phản ánh điều gì",
             case_type="FACTUAL", expect_keywords=["giông tố", "vũ trọng phụng"]),
    TestCase(case_id="TC_F12", query="Chữ người tử tù của Nguyễn Tuân kể về nhân vật nào",
             case_type="FACTUAL", expect_keywords=["chữ người tử tù", "nguyễn tuân"]),
    TestCase(case_id="TC_F13", query="Bước đường cùng của Nguyễn Công Hoan viết về ai",
             case_type="FACTUAL", expect_keywords=["bước đường cùng", "nguyễn công hoan"]),
    TestCase(case_id="TC_F14", query="Chí Phèo của Nam Cao có kết thúc như thế nào",
             case_type="FACTUAL", expect_keywords=["chí phèo", "nam cao"]),
    TestCase(case_id="TC_F15", query="Số đỏ của Vũ Trọng Phụng châm biếm điều gì",
             case_type="FACTUAL", expect_keywords=["số đỏ", "vũ trọng phụng"]),
    TestCase(case_id="TC_F16", query="Tắt đèn của Ngô Tất Tố nhân vật chính là ai",
             case_type="FACTUAL", expect_keywords=["tắt đèn", "ngô tất tố"]),

    # ── AUTHOR (+10) ── keyword = tên tác giả
    TestCase(case_id="TC_A04", query="Nguyễn Ngọc Tư có những tác phẩm nào về miền Tây",
             case_type="AUTHOR", expect_keywords=["nguyễn ngọc tư"]),
    TestCase(case_id="TC_A05", query="Hồ Biểu Chánh viết những tiểu thuyết nào",
             case_type="AUTHOR", expect_keywords=["hồ biểu chánh"]),
    TestCase(case_id="TC_A06", query="Vũ Trọng Phụng có những tác phẩm nổi tiếng nào",
             case_type="AUTHOR", expect_keywords=["vũ trọng phụng"]),
    TestCase(case_id="TC_A07", query="Ngô Tất Tố viết những tác phẩm gì",
             case_type="AUTHOR", expect_keywords=["ngô tất tố"]),
    TestCase(case_id="TC_A08", query="Kim Lân là tác giả của những truyện ngắn nào",
             case_type="AUTHOR", expect_keywords=["kim lân"]),
    TestCase(case_id="TC_A09", query="Xuân Diệu có những bài thơ tình nào",
             case_type="AUTHOR", expect_keywords=["xuân diệu"]),
    TestCase(case_id="TC_A10", query="Nguyễn Tuân viết những tác phẩm nào",
             case_type="AUTHOR", expect_keywords=["nguyễn tuân"]),
    TestCase(case_id="TC_A11", query="Nguyễn Công Hoan có những truyện châm biếm nào",
             case_type="AUTHOR", expect_keywords=["nguyễn công hoan"]),
    TestCase(case_id="TC_A12", query="Nguyễn Huy Thiệp nổi tiếng với những truyện ngắn nào",
             case_type="AUTHOR", expect_keywords=["nguyễn huy thiệp"]),
    TestCase(case_id="TC_A13", query="Duyên Anh viết về đề tài gì",
             case_type="AUTHOR", expect_keywords=["duyên anh"]),

    # ── SEMANTIC (+8) ── chủ đề / thể loại
    TestCase(case_id="TC_S04", query="Gợi ý sách về tình yêu tuổi học trò trong sáng",
             case_type="SEMANTIC", expect_keywords=["tình yêu", "học trò"]),
    TestCase(case_id="TC_S05", query="Tôi muốn đọc truyện về cuộc sống miền Tây sông nước",
             case_type="SEMANTIC", expect_keywords=["miền tây", "sông nước", "nam bộ"]),
    TestCase(case_id="TC_S06", query="Sách nào phản ánh xã hội thực dân nửa phong kiến",
             case_type="SEMANTIC", expect_keywords=["thực dân", "phong kiến", "xã hội"]),
    TestCase(case_id="TC_S07", query="Gợi ý truyện kiếm hiệp võ thuật giang hồ hấp dẫn",
             case_type="SEMANTIC", expect_keywords=["kiếm hiệp", "võ thuật", "giang hồ"]),
    TestCase(case_id="TC_S08", query="Tác phẩm nào viết về số phận người phụ nữ trong xã hội cũ",
             case_type="SEMANTIC", expect_keywords=["phụ nữ", "số phận"]),
    TestCase(case_id="TC_S09", query="Sách nào châm biếm trào phúng thói hư tật xấu",
             case_type="SEMANTIC", expect_keywords=["châm biếm", "trào phúng"]),
    TestCase(case_id="TC_S10", query="Gợi ý thơ ca cách mạng kháng chiến hào hùng",
             case_type="SEMANTIC", expect_keywords=["cách mạng", "kháng chiến"]),
    TestCase(case_id="TC_S11", query="Truyện nào viết về tình cảm gia đình cha con",
             case_type="SEMANTIC", expect_keywords=["gia đình", "cha con"]),

    # ── HARD / FALLBACK (+5) ── ngoài corpus → kỳ vọng từ chối, không hallucinate
    TestCase(case_id="TC_HARD03",
             query="Tóm tắt tiểu thuyết Trăm năm cô đơn của Gabriel Garcia Marquez",
             case_type="HARD", expect_keywords=[], expect_fallback=True,
             description="Văn học nước ngoài — ngoài corpus"),
    TestCase(case_id="TC_HARD04",
             query="Nội dung tiểu thuyết Nhà giả kim của Paulo Coelho là gì",
             case_type="HARD", expect_keywords=[], expect_fallback=True,
             description="Sách nước ngoài — ngoài corpus"),
    TestCase(case_id="TC_HARD05",
             query="Công thức tính đạo hàm của hàm số lượng giác",
             case_type="HARD", expect_keywords=[], expect_fallback=True,
             description="Toán học — ngoài phạm vi văn học"),
    TestCase(case_id="TC_HARD06",
             query="Hướng dẫn cách nấu phở bò Hà Nội ngon",
             case_type="HARD", expect_keywords=[], expect_fallback=True,
             description="Ẩm thực — ngoài phạm vi văn học"),
    TestCase(case_id="TC_HARD07",
             query="Kết quả trận đấu bóng đá World Cup gần nhất",
             case_type="HARD", expect_keywords=[], expect_fallback=True,
             description="Thể thao thời sự — ngoài corpus"),
]
